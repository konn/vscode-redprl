...
class Session {
  ...
  async execute(fileName: string): Promise<null | string> {
    if (this.config.path == null) {
      vscode.window.showWarningMessage(`The RedPRL binary path needs to be configured. See the "redprl.path" setting.`);
      return null;
    };
    ...
    return new Promise<null | string>((resolve) => {
      ...
      child.on("error", (error: Error & { code: string }) => {
        if (error.code === "ENOENT") {
          const msg = `Cannot find redprl binary at "${this.config.path}".`;
  ...
  async refreshDiagnostics(response: string): Promise<void> {
    ...
    while ((matchDiagnostic = Pattern.diagnostic.exec(response)) != null) {
      ...
      if (severity != null) {
        ...
        if (isNaN((startLine = parseInt(startLineRaw)))) continue;
        if (isNaN((startChar = parseInt(startCharRaw)))) continue;
        if (isNaN((  endLine = parseInt  (endLineRaw)))) continue;
        if (isNaN((  endChar = parseInt  (endCharRaw)))) continue;
        if (!collated.has(uri)) collated.set(uri, []);
        diagnostics = collated.get(uri) as vscode.Diagnostic[];
...
export function activate(context: vscode.ExtensionContext) {
  ...
  vscode.workspace.onDidChangeConfiguration(() => {
    (session as any).config = vscode.workspace.getConfiguration("redprl");
  });
  vscode.workspace.onDidSaveTextDocument((textDocument) => {
    if (textDocument.languageId === "redprl") {
      if (session.config.enableDiagnosticsOnSave) {
        session.refresh(textDocument);
      }
    }
  });
}
